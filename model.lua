...
return function(opt)
    ...
    function exp.test(opt, lego, model)
        ...
        -- Initial state
        lego.cur = require('lego')({input='IMAGE100.LXFML', output="out/cur.png", dim=opt.dim})
    ...
    local function create_model(opt)
        ...
        model:add(nn.SpatialConvolution(3, 32, 8, 8, 4, 4))
        -- model:add(nn.SpatialBatchNormalization(32))
        model:add(nn.ReLU(true))
        model:add(nn.SpatialConvolution(32, 64, 5, 5, 3, 3))
        -- model:add(nn.SpatialBatchNormalization(64))
        model:add(nn.ReLU(true))
        model:add(nn.SpatialConvolution(64, 64, 4, 4, 1, 1))
        -- model:add(nn.SpatialBatchNormalization(64))
        model:add(nn.ReLU(true))
        model:add(nn.View(-1, 64*6*6))
        model:add(nn.Linear(64*6*6, 512))
